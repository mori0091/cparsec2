...
- Source_new(input) ::
  ...
  - input :: a ~const char*~, ~FILE*~, or ~Source~.
  - return :: a new ~Source~ object, or ~input~ if ~input~ was a ~Source~.
- parse(p, src, ctx)    :: 
  ...
  - p   :: a parser of ~PARSER(T)~ type such as ~PARSER(Char)~, etc.
  - src :: a ~Source~ object.
  - ctx :: a potinter to ~Ctx~ object.
  - return :: an object of ~RETURN_TYPE(PARSER(T))~ type. For example,
...
- parseTest(p, text)    :: 
  ...
  - p    :: a parser of ~PARSER(T)~ type such as ~PARSER(Char)~, etc.
  - text :: a ~const char*~. (null-terminated char sequence; i.e. string in C)
...
- runParser(p, input)   ::
  ...
  - p    :: a parser of ~PARSER(T)~ type such as ~PARSER(Char)~, etc.
  - input :: a ~const char*~, ~FILE*~, or ~Source~.
  - return :: an object of ~ParseResult(T)~ type such as ~ParseResult(Char)~, etc.
...
and parser combinators:
 ...
 - *NOTE* : ~NONE~ is a value of type ~None~.
   ...
   - Parser of ~PARSER(None)~ type has no meaningful value should to be
