...
- [[#building-block-of-parser-combinators][Building block of Parser-combinators]] ::
  ...
  - PARSER_CAST(expr)
  - GENERIC_FORALL_P<n>(expr, F)
  - DECLARE_FORALL_P<n>(F)
  - DEFINE_FORALL_P<n>(F)
...
#+begin_src c
  ...
  /* Declares `PARSER(List(T)) MANY(T)(PARSER(T) p)` for all suppoting PARSER(T) */
  DECLARE_FORALL_P0(MANY);
  /*
   ,* `DECLARE_FORALL_P0(MANY);` is expanded to as follows:
   ,* ~~~c
   ,* DECLARE_MANY(Char);
   ,* DECLARE_MANY(String);
   ,* DECLARE_MANY(Int);
  ...
  /* Defines `PARSER(List(T)) MANY(T)(PARSER(T) p)` for all suppoting PARSER(T) */
  DEFINE_FORALL_P0(MANY);
  /*
   ,* `DEFINE_FORALL_P0(MANY);` is expanded to as follows:
   ,* ~~~c
   ,* DEFINE_MANY(Char);
   ,* DEFINE_MANY(String);
   ,* DEFINE_MANY(Int);
...
~GENERIC_FORALL_P<n>(expr, F)~ is a macro function to define generic
parser-combinator function. (it is expanded to "C11 _Generic selection"
expression.)
- if ~expr~ was a parser of supported ~PARSER(T)~ type, returns ~F(T)~.
- where ~T~ is one of the following (~<n>~ must be 0 or 1):
  - ~Char~
  - ~String~
  - ~Int~
  - ~List(String)~ (only if 0 \lt ~<n>~)
  - ~List(Int)~ (only if 0 \lt ~<n>~)

** DECLARE_FORALL_P<n>(F)

~DECLARE_FORALL_P<n>(F)~ is a macro function to declare function prototypes of
concrete parser-combinator ~F(T)~ for all supported ~PARSER(T)~ type.

This macro is expanded to ~DECLARE_F(T);~ statements for all supported
~PARSER(T)~ type. Therefore the ~F(T)~ and ~DECLARE_F(T)~ macro function must be
defined before.
- ~F(T)~ shall be expanded to the name of a concrete parser-combinator.
- ~DECLARE_F(T)~ shall be expanded to the function prototype of ~F(T)~.
- where ~T~ is one of the following (~<n>~ must be 0 or 1):
  - ~Char~
  - ~String~
  - ~Int~
...
~DEFINE_FORALL_P<n>(F)~ is a macro function to define function of concrete
parser-combinator ~F(T)~ for all supported ~PARSER(T)~ type.

This macro is expanded to ~DEFINE_F(T);~ statements for all supported
~PARSER(T)~ type. Therefore the ~F(T)~ and ~DEFINE_F(T)~ macro function must be
defined before.
- ~F(T)~ shall be expanded to the name of a concrete parser-combinator.
- ~DEFINE_F(T)~ shall be expanded to the function definition of ~F(T)~.
- where ~T~ is one of the following (~<n>~ must be 0 or 1):
  - ~Char~
  - ~String~
  - ~Int~
