...
*NOTE* : ~List(T)~ must be one of a following:
  - ~List(Char)~ (whose element type ~E~ is ~char~)
  - ~List(String)~ (whose element type ~E~ is ~const char*~)
  - ~List(Int)~ (whose element type ~E~ is ~int~)

*NOTE* : ~List(Char)~ is same as ~char*~ (i.e. string in C)

To iterate elements contained in a ~List(T)~ object, use the following APIs.\\
*NOTE* : ~E~ is the type of element of ~List(T)~ object.
...
#+begin_src c
  ...
  char* itr = list_begin(xs);
  char* end = list_end(xs);
  while (itr != end) {
    printf("%c\n", *itr);
    itr++;
  }
...
*NOTE* : ~Buff(T)~ must be one of a following:
  - ~Buff(Char)~ (whose element type ~E~ is ~char~)
  - ~Buff(String)~ (whose element type ~E~ is ~const char*~)
  - ~Buff(Int)~ (whose element type ~E~ is ~int~)

To build a ~List(T)~ object, use the following APIs:\\
*NOTE* : ~E~ is the type of element of ~Buff(T)~ object and resulting ~List(T)~ object.
