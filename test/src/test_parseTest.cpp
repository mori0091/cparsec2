...
SCENARIO("parseTest(p, str)", "[cparsec2][core][parseTest]") {
  ...
  WHEN("parseTest(digit, \"123\")") {
    THEN("results true") {
      REQUIRE(parseTest(digit, "123"));
    }
  }
  WHEN("parseTest(alpha, \"123\")") {
    THEN("results false") {
      REQUIRE_FALSE(parseTest(alpha, "123"));
    }
  }
  // StringParser
  WHEN("parseTest(many1(digit), \"123\")") {
    THEN("results true") {
      REQUIRE(parseTest(many1(digit), "123"));
    }
  }
...
SCENARIO("PARSE_TEST(p, str)", "[cparsec2][core][PARSE_TEST]") {
  ...
  WHEN("PARSE_TEST(digit, \"123\")") {
    THEN("results true") {
      REQUIRE(PARSE_TEST(digit, "123"));
    }
  }
  WHEN("PARSE_TEST(alpha, \"123\")") {
    THEN("results false") {
      REQUIRE_FALSE(PARSE_TEST(alpha, "123"));
    }
  }
  // StringParser
  WHEN("PARSE_TEST(many1(digit), \"123\")") {
    THEN("results true") {
      REQUIRE(PARSE_TEST(many1(digit), "123"));
    }
  }
